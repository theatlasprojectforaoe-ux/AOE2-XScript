// 数组管理列表 array_list: ID编号为0的数组。该数组用于管理场景中创建的其它数组ID和数组名称
...
{// 创建数组，将数组名称加入管理列表
    ...
    else {xsChatData("Create array failed, invalid dtype."); return (-32768);}
    ...
    if(xsArrayGetString(array_list, arr_id) == "") {xsArraySetString(array_list, arr_id, arr_name);
    //xsChatData("Register array(%d) successed.", arr_id);
    }
    else {xsChatData("Register array(%d) failed, the array already exists.", arr_id); return (-32768);}
    return (arr_id);
}
...
*/
    array_list = xsArrayCreateString(1, "", "array_list");
    xsArraySetString(array_list, 0, "Arrays");
    // 用于缓存变量的 值数组 ，索引数组
...
//include "array_tools.xs";    // 包含数组处理模块

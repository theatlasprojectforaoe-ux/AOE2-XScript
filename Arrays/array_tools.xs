...
/// 数组统计相关 ///
int IntArrChoice(int arr_id=-1, int start_idx=0, int end_idx=-1) 
{
/*
...
float FloatArrChoice(int arr_id=-1, int start_idx=0, int end_idx=-1) 
{
/*
...
string StringArrChoice(int arr_id=-1, int start_idx=0, int end_idx=-1) 
{
/*
...
vector VectorArrChoice(int arr_id=-1, int start_idx=0, int end_idx=-1) 
{
/*
...
/**
 * @brief 枚举整型数组在区间 [start_idx, end_idx) 上的元素
 * @param arr_id: 要枚举元素的数组ID
 * @param start_idx: 枚举的起始索引位置
 * @param end_idx: 枚举的结束索引位置
 * @param step: 枚举步长。每移动step个索引位置，对元素进行枚举
 * @return <Int> 存储枚举元素集的数组ID
**/
int IntArrEnum(int arr_id=-1, int start_idx=0, int end_idx=-1, int step=1, int temp_arr=-1) 
{
    int arr_size = xsArrayGetSize(arr_id);
    if(end_idx==-1) {end_idx = arr_size-1;}
    if(step > arr_size || step <= 0-arr_size) {xsChatData("invalid step, exit Enum."); return (-1);}
    else {
        if(temp_arr < 0) {temp_arr = xsArrayCreateInt(end_idx-start_idx, 0, "IArr_temp_"+iarr_temp); iarr_temp++;}
        if(step > 0) {
            while(start_idx < end_idx) 
            {
                xsArraySetInt(temp_arr, start_idx, xsArrayGetInt(arr_id, start_idx));
        ...
        if(step < 0) {
            while(start_idx > end_idx) 
            {
                xsArraySetInt(temp_arr, end_idx, xsArrayGetInt(arr_id, end_idx));
...
/**
 * @brief 枚举浮点型数组在区间 [start_idx, end_idx) 上的元素
 * @param arr_id: 要枚举元素的数组ID
 * @param start_idx: 枚举的起始索引位置
 * @param end_idx: 枚举的结束索引位置
 * @param step: 枚举步长。每移动step个索引位置，对元素进行枚举
 * @return <Int> 存储枚举元素集的数组ID
**/
int FloatArrEnum(int arr_id=-1, int start_idx=0, int end_idx=-1, int step=1, int temp_arr=-1) 
{
    int arr_size = xsArrayGetSize(arr_id);
    if(end_idx==-1) {end_idx = arr_size-1;}
    if(step > arr_size || step <= 0-arr_size) {xsChatData("invalid step, exit Enum."); return (-1);}
    else {
        if(temp_arr < 0) {temp_arr = xsArrayCreateFloat(end_idx-start_idx, 0.0, "FArr_temp_"+farr_temp); farr_temp++;}
        if(step > 0) {
            while(start_idx < end_idx) 
            {
                xsArraySetFloat(temp_arr, start_idx, xsArrayGetFloat(arr_id, start_idx));
                start_idx = start_idx + step;
            }
        }
        if(step < 0) {
            while(start_idx > end_idx) 
            {
                xsArraySetFloat(temp_arr, end_idx, xsArrayGetFloat(arr_id, end_idx));
                end_idx = end_idx + step;
            }
        }
        return (temp_arr);
